...
- 단일 룰 탐색
- monthly crash 필터 추가
- walk-forward crash split 필터 추가
- final_wf_filtered 룰 출력
- 최종 실전 후보를 11_final_rules_wf_filtered.csv로 분리
- OR 룰셋 없음

v2에서는 month_crash_precision, max_valid_crash_months, wf_crash_precision, final_max_wf_crash_splits 같은 옵션이 추가됐습니다. 
즉, 전체 valid precision이 높아도 특정 월이나 walk-forward 구간에서 크게 무너지면 제외합니다
...
def default_extra_thresholds() -> Dict[str, List[Tuple[str, float]]]:
    return {
        "room_to_20d_high": [
            ...
            ("<=", 0.0), ("<=", 3.0), ("<=", 4.4438), ("<=", 5.0), ("<=", 10.0),
            ("<=", 20.0), ("<=", 30.0),
        ],
        "room_to_60d_high": [
            (">=", -10.0), (">=", -5.0), (">=", 0.0), (">=", 3.0), (">=", 5.0),
            ("<=", 0.0), ("<=", 5.0), ("<=", 10.0), ("<=", 12.2626),
            ("<=", 18.9896), ("<=", 20.0), ("<=", 30.0), ("<=", 50.0),
        ],
        "upper_wick_ratio": [
            ("<=", 0.0), ("<=", 0.02), ("<=", 0.04), ("<=", 0.064),
...
def main():
    ...
    parser.add_argument("--out", default="stable_rule_miner_final_v2_out")
    ...
    print("[INFO] goal: valid 70% + count + no monthly/wf crash")
    print("=" * 80)
...
"""
기본
python stable_rule_miner_final_v2.py \
  --csv csv/low_result_7_desc.csv \
  --out stable_rule_miner_final_v2_out \
  --date-col today \
  --max-depth 6 \
  --beam-width 1500 \
  --top-k 150 \
...
python stable_rule_miner_final_v2.py \
  --csv csv/low_result_7_desc.csv \
  --out stable_rule_miner_final_v2_loose \
  --date-col today \
  --max-depth 6 \
  --beam-width 1500 \
  --top-k 150 \
  ...
  --min-train-lift 1.20 \
  --min-valid-lift 1.45 \
  --max-precision-gap 0.20 \
  --min-month-count 5 \
  --month-pass-precision 0.52 \
  --month-crash-precision 0.40 \
  --min-valid-month-min-precision 0.50 \
  --max-valid-crash-months 1 \
  --valid-count-weight 8 \
  --wf-top-rules 150 \
  --wf-min-precision 0.58 \
  --wf-crash-precision 0.40 \
  --final-min-valid-precision 0.68 \
  --final-min-valid-count 50 \
  --final-min-wf-mean-precision 0.68 \
  --final-min-wf-min-precision 0.50 \
  --final-min-wf-pass-split-rate 0.60 \
  --final-min-wf-total-count 50 \
  --final-max-wf-crash-splits 1 \
  --simplify
"""