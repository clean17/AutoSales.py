...
"""
15개 저점반등 피쳐 전용 룰/피쳐 유용성 평가 스크립트.

목적:
- target_before_stop_10 / target_before_stop_15 공통 룰 마이닝용 15개 피쳐만 사용
- 삭제된 과거 피쳐가 ALLOWED_OPS, threshold, non-monotonic, regime 목록에 남지 않도록 정리
- 단독 성능이 약한 피쳐도 룰 조합에서 조건부 필터로 평가
"""
...
def main():
    ...
    print("[SCRIPT_VERSION] feature_selector_v36_ev_min_features_aligned15")
...
# ============================================================
"""
python low\feature_selector_v36_ev.py ^
  --csv csv\low_result_7_v2_desc.csv ^
  --out feature_selector_v36_ev_out ^
  --date-col today ^
  --max-depth 5 ^
  --beam-width 500 ^
  --top-k 150 ^
...
python low\feature_selector_v36_ev.py ^
  --csv csv\low_result_7_v2_desc.csv ^
  --out feature_selector_v36_ev_out_wide ^
  --date-col today ^
  --max-depth 6 ^
  --beam-width 3000 ^
  --top-k 200 ^
