...
def mine_avoid_rules(
    ...
    beams = [(np.ones(len(df), dtype=bool), [], -1)]
    good = {}
    ...
    print(
        ...
        "\nmin_added_total", MIN_ADDED_TOTAL,
        "\n"
    )
    ...
    for depth in range(max_depth):
        ...
        for base_mask, conds, last_lit_idx in beams:
            ...
            for lit_idx, (lit, lmask) in enumerate(zip(literals, literal_masks)):
                if lit_idx <= last_lit_idx:
                    continue
                ...
                if (
                    ...
                    if key2 in good:
                        save_stats["duplicate_save"] += 1
                    else:
                        save_stats["unique_save"] += 1
                ...
                if (
                    ...
                    item = (k, next(uid), m, conds + [lit], lit_idx, bad_rate, cnt)
        ...
        print("[AVOID] raw unique good:", len(good))
        ...
        tail = new[-1]
        print("[AVOID] tail bad_rate:", round(tail[5], 3), "cnt:", tail[6], "conds:", tail[3])

        beams = [(m, conds, lit_idx) for _, _, m, conds, lit_idx, _, _ in new]
...
def select_avoid_rules_max_class0_under_class3_limit(
    ...
    while True:
        ...
        for cand in candidates:
            ...
            if added_total < MIN_ADDED_TOTAL:
            ...
            if added_class0_rate < 0.75:
                reject_stats["class0_rate"] += 1
                continue

            if added_protect_rate > 0.16:
                reject_stats["protect_rate"] += 1
                continue

            if added_class3_rate > 0.04:
                print(
                    "[REJECT class3]",
                    cand["name"],
                    "total=", added_total,
                    ...
                    "c2=", added_class2,
                    "c3=", added_class3,
                    "c0_rate=", round(added_class0_rate, 3),
                    "protect_rate=", round(added_protect_rate, 3),
                    "c3_rate=", round(added_class3_rate, 3),
                )
